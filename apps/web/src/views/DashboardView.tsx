  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass-card-light p-5 rounded-2xl border border-slate-200 flex items-center justify-between">
          <div>
            <div className="text-xs font-semibold text-slate-500">Registered Active Patients</div>
            ...
            <div className="text-[11px] font-bold text-emerald-600 mt-1 flex items-center gap-1">
              <TrendingUp className="w-3 h-3" /> +14% active encounters
            </div>
          </div>
        ...
        <div className="glass-card-light p-5 rounded-2xl border border-slate-200 flex items-center justify-between">
          <div>
            <div className="text-xs font-semibold text-slate-500">Clinical Medical Specialists</div>
            <div className="text-2xl font-black text-slate-900 mt-1">{kpis.totalDoctors}</div>
            <div className="text-[11px] font-medium text-slate-500 mt-1">Across 5 Departments</div>
          </div>
        ...
        <div className="glass-card-light p-5 rounded-2xl border border-slate-200 flex items-center justify-between">
          <div>
            <div className="text-xs font-semibold text-slate-500">Hospital Revenue Collected</div>
            <div className="text-2xl font-black text-slate-900 mt-1">${kpis.totalRevenue.toLocaleString()}</div>
            <div className="text-[11px] font-bold text-emerald-600 mt-1 flex items-center gap-1">
              <TrendingUp className="w-3 h-3" /> Stripe & Cash Ledger
            </div>
          </div>
        ...
        <div className="glass-card-light p-5 rounded-2xl border border-slate-200 flex items-center justify-between">
          <div>
            <div className="text-xs font-semibold text-slate-500">Ward & ICU Bed Occupancy</div>
            ...
            <div className="text-[11px] font-bold text-amber-600 mt-1 flex items-center gap-1">
              <Bed className="w-3 h-3" /> {Math.round((kpis.occupiedBeds / kpis.totalBeds) * 100)}% Bed Capacity
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-card-light p-5 rounded-2xl border border-slate-200 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              <h3 className="font-bold text-sm text-slate-900">Weekly Patient Encounters Velocity</h3>
              <p className="text-[11px] text-slate-500">Outpatient consultations and OPD volume telemetry</p>
            </div>
        ...
        <div className="glass-card-light p-5 rounded-2xl border border-slate-200 space-y-4">
          <div className="border-b border-slate-100 pb-3">
            <h3 className="font-bold text-sm text-slate-900">Department Occupancy Rates</h3>
            <p className="text-[11px] text-slate-500">Clinical ward & specialty bed allocation</p>
          </div>
            ...
            {depts.map((d: any) => (
              <div key={d.name} className="space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold text-slate-600">{d.occupancyRate}% Beds</span>
                </div>
                <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      d.occupancyRate > 85 ? 'bg-rose-500' : d.occupancyRate > 70 ? 'bg-amber-500' : 'bg-teal-500'
                    }`}
                    style={{ width: `${d.occupancyRate}%` }}
                  ></div>
                </div>
                <div className="text-[10px] text-slate-400 font-medium">{d.activeDoctors} On-Duty Specialists</div>
              </div>
            ))}
          </div>
        </div>
